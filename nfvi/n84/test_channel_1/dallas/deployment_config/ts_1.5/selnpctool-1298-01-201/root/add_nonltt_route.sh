...
ip add add 11.0.30.1/24 dev vlan0130 || true
ip add add 2a00:11:0:30::3/64 dev vlan0130 || true
ip add add 11.0.29.1/24 dev vlan0129 || true
ip add add 11.0.5.1/24 dev vlan0105 || true
ip add add 2a00:11:0:5::3/64 dev vlan0105 || true
ip add add 11.0.4.1/24 dev vlan0104 || true
ip add add 11.0.2.1/24 dev vlan0102 || true
...
ip link set p1p1 up || true
ip link set p2p1 up || true
...
ip route add 6.192.0.0/19 via 11.0.2.3 || true
ip route add 6.196.0.0/19 via 11.0.2.3 || true
ip route add 6.192.32.0/19 via 11.0.2.4 || true
ip route add 6.196.32.0/19 via 11.0.2.4 || true

ip route add 8.140.0.0/19 via 11.0.2.3 || true
ip route add 8.140.32.0/19 via 11.0.2.4 || true

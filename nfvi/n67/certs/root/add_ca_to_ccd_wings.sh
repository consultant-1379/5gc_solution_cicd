...
echo -e -n "INFO: Upload Root CA to node director ${remote_path}...\t"
if [ ! -f /etc/pki/trust/anchors/RootCA.crt ];then
...
do
    ...
    echo -e -n "INFO: Upload Root CA to node ${node_name} ${remote_path}...\t"
    if ssh -q -o StrictHostKeyChecking=no ${node_ip} "test -f ${remote_path}/RootCA.crt";then
      echo "[Skipped]"
      continue
    fi
    scp -q -o StrictHostKeyChecking=no RootCA.crt ${node_ip}:~/
    ssh -q -o StrictHostKeyChecking=no ${node_ip} "sudo mv ~/RootCA.crt ${remote_path}"
    echo "[OK]"
    if [ $res -eq 0 ];then

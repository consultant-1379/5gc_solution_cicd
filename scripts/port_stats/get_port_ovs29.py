...
neutron_port_list = os.popen("neutron port-list -c name -c binding:vif_details -f json -q").read()

dpctl_show = os.popen("ssh -q {0}@{1} 'sudo ovs-appctl dpctl/show | grep -v tap'".format(username, hostname)).read().splitlines()

pmd_rxq_show = os.popen("ssh -q {0}@{1} 'sudo ovs-appctl dpif-netdev/pmd-rxq-show'".format(username, hostname)).read().splitlines()
...
for port in neutron_port_dict_list:
    if port["binding:vif_details"].has_key("vhostuser_socket"):
        ovs_port_name = port["binding:vif_details"]["vhostuser_socket"].split("/")[3]
        ovs_port_info = {}
    ...
    if 'pmd thread' in line:
        ...
        if m:
            ...
            if ovs_port_name in ovs_port_all.keys():
                ovs_port_all[ovs_port_name]["pmd_cpu"] = "{0}.{1}({2}%)".format(numa_id, cpu_id, pmd_usage)
...
for ovs_port_name in ovs_port_list:
    if ovs_port_all[ovs_port_name].has_key("port_num"):
        ...
        continue
        print "{0:17}{1:4}{2:12}{3:20}".format(ovs_port_name, ovs_port_all[ovs_port_name]["port_num"], ovs_port_all[ovs_port_name]["pmd_cpu"], ovs_port_all[ovs_port_name]["neutron_port"])
print "-------------------------------------------------------"

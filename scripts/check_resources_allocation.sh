...
print_resource_allocation()
{
    ...
    echo "Namespace CPU_Req CPU_Limit MEM_Req(Gi) MEM_Limit(Gi) Disk(Gi)"
    ...
    do
        ...
        echo ${ns} ${sum_cpu_req} ${sum_cpu_limit} $(echo "${sum_mem_req} / 1024 / 1024 / 1024" | bc) $(echo "${sum_mem_limit} / 1024 / 1024 / 1024" | bc) ${sum_disk}
    done

    if [[ ${num} -gt 1 ]]; then
        echo SUM: ${sum_cluster_cpu_req} ${sum_cluster_cpu_limit} $(echo "${sum_cluster_mem_req} / 1024 / 1024 / 1024" | bc) $(echo "${sum_cluster_mem_limit} / 1024 / 1024 / 1024" | bc) ${sum_cluster_disk}
    fi
}

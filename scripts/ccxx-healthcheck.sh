...
function check_alarms() {
  ...
  then 
    ...
    CCXX_NS=$1
    alarms_list=$(kubectl -n ${CCXX_NS} exec -it $(kubectl -n ${CCXX_NS} get pods | grep fh-alarm | grep -v topics | tail -1 | awk '{print $1}') -c eric-fh-alarm-handler -- curl http://eric-fh-alarm-handler:5005/ah/api/v0/alarms?outputFormat=FullAlarmList | jq . |tee -a $REPORT_FILE ) 
...
then 
  ...
  log_info "### Summary: Congratulations!, The $ns is in Healthy State(NOTE:Alarm not checked for now ,need update later!!!)." |tee -a $REPORT_FILE
